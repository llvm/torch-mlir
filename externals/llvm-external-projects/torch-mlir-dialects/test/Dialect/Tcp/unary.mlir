...
func.func @test_cast_i32_f32(%arg0 : tensor<?x?xi32>) -> tensor<?x?xf32> {
  // expected-error@+1{{tcp.cast' op in_int_element_type attr must be set when input is INT}}
...
func.func @test_cast_i32_f32(%arg0 : tensor<?x?xf32>) -> tensor<?x?xi16> {
  // expected-error@+1{{tcp.cast' op out_int_element_type attr must be set when output is INT}}
...
// CHECK:         %[[OUT:.*]] = tcp.cast %[[ARG]] {in_int_element_type = 1 : ui32, out_int_element_type = 2 : ui32} : tensor<?x?xi32> -> tensor<?x?xi8>
// CHECK:         return %[[OUT]] : tensor<?x?xi8>
func.func @test_cast_i32_i8(%arg0 : tensor<?x?xi32>) -> tensor<?x?xi8> {
  %0 = tcp.cast %arg0 {in_int_element_type = 1 : ui32, out_int_element_type = 2 : ui32} : tensor<?x?xi32> -> tensor<?x?xi8>
...
// CHECK:         %[[OUT:.*]] = tcp.cast %[[ARG]] {in_int_element_type = 2 : ui32} : tensor<?x?xi32> -> tensor<?x?xf16>
// CHECK:         return %[[OUT]] : tensor<?x?xf16>
func.func @test_cast_i32_f16(%arg0 : tensor<?x?xi32>) -> tensor<?x?xf16> {
  %0 = tcp.cast %arg0 {in_int_element_type = 2 : ui32} : tensor<?x?xi32> -> tensor<?x?xf16>
...
// CHECK:         %[[OUT:.*]] = tcp.cast %[[ARG]] {out_int_element_type = 0 : ui32} : tensor<?x?xf16> -> tensor<?x?xi32>
// CHECK:         return %[[OUT]] : tensor<?x?xi32>
func.func @test_cast_f16_i32(%arg0 : tensor<?x?xf16>) -> tensor<?x?xi32> {
  %0 = tcp.cast %arg0 {out_int_element_type = 0 : ui32} : tensor<?x?xf16> -> tensor<?x?xi32>
  return %0 : tensor<?x?xi32>
}
