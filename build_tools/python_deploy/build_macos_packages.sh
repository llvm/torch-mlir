    ...
    for python_version in $python_versions; do
      ...
      case "$package" in
        ...
        torch-mlir-no-jit-importer)
          clean_wheels torch_mlir_no_jit_importer "$python_version"
          build_torch_mlir_no_jit_importer torch_mlir_no_jit_importer "$python_version"
          run_audit_wheel_no_jit_importer torch_mlir_no_jit_importer "$python_version"
          ;;
        *)
...
function build_torch_mlir_no_jit_importer() {
  local wheel_basename="$1"
  local python_version="$2"
  rm -rf "$output_dir"/build_venv
  python"${python_version}" -m venv "$output_dir"/build_venv
...
function run_audit_wheel_no_jit_importer() {
  set +x
  local wheel_basename="$1"
  local python_version="$2"
  generic_wheel=$(ls "$output_dir"/"${wheel_basename}"-* | grep "${python_version//./}")
