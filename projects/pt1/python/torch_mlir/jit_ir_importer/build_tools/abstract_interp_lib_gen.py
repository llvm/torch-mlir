...
def aten〇softplus_backward〡shape(grad_output: List[int], self: List[int], beta: float, threshold: float) -> List[int]:
    return upstream_shape_functions.unary(grad_output)
...
def prims〇prod〡shape(inp: List[int], dims: Optional[List[int]], output_dtype: Optional[int] = None) -> List[int]:
    return upstream_shape_functions.sum_mean_dim(inp, dims, False, output_dtype)
...
def avg_pool3d(input: List[int], kernel_size: List[int], stride: List[int], padding: List[int], ceil_mode: bool, count_include_pad: bool, divisor_override: Optional[int]):
    assert (
        len(kernel_size) == 1 or len(kernel_size) == 3
    ), "max_pool3d: kernel_size must either be a single int, or a tuple of three ints"
    (kD, kH, kW) = (kernel_size[0], kernel_size[0], kernel_size[0]) if len(kernel_size) == 1 else (kernel_size[0], kernel_size[1], kernel_size[2])
...
def aten〇avg_pool3d〡shape(self: List[int], kernel_size: List[int], stride: List[int] = (), padding: List[int] = (0, 0, 0,), ceil_mode: bool = False, count_include_pad: bool = True, divisor_override: Optional[int] = None) -> List[int]:
    return avg_pool3d(self, kernel_size, stride, padding, ceil_mode, count_include_pad, divisor_override)
...
def aten〇empty_strided〡shape(size: List[int], stride: List[int], dtype: Optional[int] = None, layout: Optional[int] = None, device: Optional[device] = None, pin_memory: Optional[bool] = None) -> List[int]:
    return size
def aten〇empty_permuted〡shape(size: List[int], physical_layout: List[int], dtype: Optional[int] = None, layout: Optional[int] = None, device: Optional[device] = None, pin_memory: Optional[bool] = None) -> List[int]:
    return size
...
@check_dtype_function(_check_tensors_with_the_same_dtype(num_of_tensors=1, dims=[0]))
def prims〇prod〡dtype(inp_rank_dtype: Tuple[int, int], dims: Optional[List[int]], output_dtype: Optional[int] = None) -> int:
    # When invoking prims.prod() with the output_dtype argument, pytorch
    # complains that the argument is not known.
    assert output_dtype is None
    inp_rank, inp_dtype = inp_rank_dtype
    return inp_dtype
...
def aten〇empty_strided〡dtype(size: List[int], stride: List[int], dtype: Optional[int] = None, layout: Optional[int] = None, device: Optional[device] = None, pin_memory: Optional[bool] = None) -> int:
    return torch.float32 if dtype is None else dtype
@check_dtype_function(
    _check_tensors_with_the_same_dtype(num_of_tensors=0, size=[1], physical_layout=[0]) +
    _check_tensors_with_the_same_dtype(num_of_tensors=0, size=[1], physical_layout=[0], dtype=torch.float16) +
    _check_tensors_with_the_same_dtype(num_of_tensors=0, size=[1], physical_layout=[0], dtype=torch.int32) +
    _check_tensors_with_the_same_dtype(num_of_tensors=0, size=[1], physical_layout=[0], dtype=torch.complex64))
def aten〇empty_permuted〡dtype(size: List[int], physical_layout: List[int], dtype: Optional[int] = None, layout: Optional[int] = None, device: Optional[device] = None, pin_memory: Optional[bool] = None) -> int:
    return torch.float32 if dtype is None else dtype
@check_dtype_function(
