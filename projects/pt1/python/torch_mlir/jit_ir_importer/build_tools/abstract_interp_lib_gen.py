...
_index_add_invocations = [
    # same dtype
    Invocation(TensorOfShape(3, 4, dtype=dtype), 0, TensorOfShape(2, dtype=torch.int64), TensorOfShape(2, 4, dtype=dtype))
    for dtype in _SORTED_TORCH_TYPES
] + [
    # self/source different dtypes — output dtype should still follow self
    Invocation(TensorOfShape(3, 4, dtype=dtype), 0, TensorOfShape(2, dtype=torch.int64), TensorOfShape(2, 4, dtype=torch.float32))
    for dtype in _SORTED_TORCH_TYPES
] + [
    # index dtype shouldn't affect output dtype
    Invocation(TensorOfShape(3, 4, dtype=torch.float32), 0, TensorOfShape(2, dtype=dtype), TensorOfShape(2, 4, dtype=torch.float32))
    for dtype in _SORTED_TORCH_TYPES
]

@check_dtype_function(_index_add_invocations)
