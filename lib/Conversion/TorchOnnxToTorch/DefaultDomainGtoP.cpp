  ...
  patterns.onOp(
      ...
      [](OpBinder binder, ConversionPatternRewriter &rewriter) {
        ...
        if (doRotary) {
          ...
          kRotary = rewriter.create<Torch::OnnxVariantAtenRotaryEmbeddingOp>(
        ...
        Value presentKey = rewriter.create<Torch::AtenCatOp>(
            loc, resultTypes[1], keyList, cstConcatDim);

        Value valueList = rewriter.create<Torch::PrimListConstructOp>(
            loc, kvListType, SmallVector<Value>{pastValue, vInput});
        Value presentValue = rewriter.create<Torch::AtenCatOp>(
