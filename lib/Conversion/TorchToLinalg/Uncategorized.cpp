...
static Value getElementFromSingleElementTensor(OpBuilder &b, Location loc,
                                               Value tensor) {
  int64_t tensorRank = tensor.getType().cast<RankedTensorType>().getRank();
  Value zero = getConstant(b, loc, 0, b.getIndexType());
  SmallVector<Value> indices(tensorRank, zero);
  return b.create<tensor::ExtractOp>(loc, tensor, indices);
}
...
public:
  ...
  matchAndRewrite(AtenNllLossBackwardOp op, OpAdaptor adaptor,
    ...
    // becomes zero.
    Value gradInput = createElementwiseLinalgGeneric(
        rewriter, loc, {input}, resultElementType,
        [&](OpBuilder &b, Location loc, ValueRange _) {
          Value classIndex = b.create<linalg::IndexOp>(loc, inputRank - 1);

          Value targetElem;
          if (inputRank == 2) {
            Value batchIndex = b.create<linalg::IndexOp>(loc, 0);
            targetElem = b.create<tensor::ExtractOp>(loc, target, batchIndex);
          } else {
            targetElem = getElementFromSingleElementTensor(b, loc, target);
          }
          targetElem = castIntToIndex(b, loc, targetElem);

          Value gradOutElem;
          if (reduction == torch_upstream::Reduction::None && inputRank == 2) {
            Value batchIndex = b.create<linalg::IndexOp>(loc, 0);
            gradOutElem =
                b.create<tensor::ExtractOp>(loc, gradOutput, batchIndex);
          } else {
            gradOutElem = getElementFromSingleElementTensor(b, loc, gradOutput);
          }

          if (reduction == torch_upstream::Reduction::Mean) {
            Value totalWeightElem =
                getElementFromSingleElementTensor(rewriter, loc, totalWeight);
            gradOutElem =
                b.create<arith::DivFOp>(loc, gradOutElem, totalWeightElem);
          }

          Value negGradOutElem = b.create<arith::NegFOp>(loc, gradOutElem);
          Value weightElem = getConstant(b, loc, 1, resultElementType);
          if (!weightIsNone)
            weightElem = b.create<tensor::ExtractOp>(loc, weight, targetElem);
