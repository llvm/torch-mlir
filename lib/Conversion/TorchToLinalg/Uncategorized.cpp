...
static Value NearestInterpolate(OpBuilder &b, Location loc, Value outputSizeH,
  ...
  Value inputH = b.create<arith::SubFOp>(loc, inputHFP, cstOneFloat);
  Value inputW = b.create<arith::SubFOp>(loc, inputWFP, cstOneFloat);

  Value outputSizeHFP =
      b.create<arith::SIToFPOp>(loc, b.getF32Type(), outputSizeH);
  Value outputSizeWFP =
      b.create<arith::SIToFPOp>(loc, b.getF32Type(), outputSizeW);
  Value hStep = b.create<arith::DivFOp>(loc, inputH, outputSizeHFP);
  Value wStep = b.create<arith::DivFOp>(loc, inputW, outputSizeWFP);
  ...
  Value yProj = b.create<arith::MulFOp>(loc, hStep, yOutFP);
  ...
  Value xProj = b.create<arith::MulFOp>(loc, wStep, xOutFP);

  Value yNearestFP = b.create<math::RoundOp>(loc, yProj);
  Value xNearestFP = b.create<math::RoundOp>(loc, xProj);
...
namespace {
class ConvertInterpolateNearestOp
        ...
        rewriter
            .create<linalg::GenericOp>(
                ...
                [&](OpBuilder &b, Location loc, ValueRange args) {
                  ...
                  Value retVal = NearestInterpolate(
                      b, loc, outputSizeIntValues[0], outputSizeIntValues[1],
...
static Value BilinearInterpolate(OpBuilder &b, Location loc, Value outputSizeH,
                                 Value outputSizeW, Value input,
                                 Value inputSizeH, Value inputSizeW) {
  int hDimOffset = 2;
  auto inputType = input.getType().cast<RankedTensorType>();
  ...
  Value cstOne = b.create<arith::ConstantOp>(loc, b.getI64IntegerAttr(1));
  Value cstOneFloat = b.create<arith::ConstantOp>(loc, b.getF32FloatAttr(1.0));
  Value inputH = b.create<arith::SubIOp>(loc, inputSizeH, cstOne);
  Value inputW = b.create<arith::SubIOp>(loc, inputSizeW, cstOne);

  Value yOut = b.create<linalg::IndexOp>(loc, 2);
  Value xOut = b.create<linalg::IndexOp>(loc, 3);

  Value inputHFP = b.create<arith::SIToFPOp>(loc, b.getF32Type(), inputH);
  Value outputSizeHFP =
      b.create<arith::SIToFPOp>(loc, b.getF32Type(), outputSizeH);
  Value hStep = b.create<arith::DivFOp>(loc, inputHFP, outputSizeHFP);
  Value yOutInt = b.create<arith::IndexCastOp>(loc, b.getI64Type(), yOut);
  Value yOutFP = b.create<arith::SIToFPOp>(loc, b.getF32Type(), yOutInt);
  Value yProj = b.create<arith::MulFOp>(loc, hStep, yOutFP);

  Value inputWFP = b.create<arith::SIToFPOp>(loc, b.getF32Type(), inputW);
  Value outputSizeWFP =
      b.create<arith::SIToFPOp>(loc, b.getF32Type(), outputSizeW);
  Value wStep = b.create<arith::DivFOp>(loc, inputWFP, outputSizeWFP);
  Value xOutInt = b.create<arith::IndexCastOp>(loc, b.getI64Type(), xOut);
  Value xOutFP = b.create<arith::SIToFPOp>(loc, b.getF32Type(), xOutInt);
  Value xProj = b.create<arith::MulFOp>(loc, wStep, xOutFP);
...
namespace {
class ConvertBilinearInterpolateOp
    : public OpConversionPattern<Aten__InterpolateSizeListScaleListOp> {
public:
  using OpConversionPattern::OpConversionPattern;
  LogicalResult
        ...
        rewriter
            .create<linalg::GenericOp>(
                ...
                [&](OpBuilder &b, Location loc, ValueRange args) {
                  ...
                  Value retVal =
                      BilinearInterpolate(b, loc, outputSizeH, outputSizeW,
                                          input, inputSizeH, inputSizeW);
                  b.create<linalg::YieldOp>(loc, retVal);
                })
            .getResult(0);
...
void mlir::torch::torch_to_linalg::populateUncategorizedPatternsAndLegality(
  ...
  patterns.add<ConvertBilinearInterpolateOp>(typeConverter, context);
  patterns.add<ConvertInterpolateNearestOp>(typeConverter, context);
}
