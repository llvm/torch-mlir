...
Value createRemainderPayload(OpBuilder &b, Location loc,
                             ...
                             ValueRange payloadArgs, OpT op,
                             ArrayRef<Value> operands) {
  static_assert(std::is_same_v<OpT, AtenRemainderScalarOp> ||
                    std::is_same_v<OpT, AtenRemainderTensorOp>,
                "template type must be a tensor/scalar remainder");
  typename OpT::Adaptor adaptor(operands);
  Type dtype = cast<RankedTensorType>(converter->convertType(op.getType()))
                   .getElementType();
  Value lhs = convertScalarToDtype(b, loc, payloadArgs[0], dtype);
  ...
  if (isa<mlir::FloatType>(dtype)) {
    ...
    result = b.create<scf::IfOp>(
                  loc, condition,
                  // if branch
                  [&](OpBuilder &b, Location loc) {
                    Value fixedRemainder =
                        b.create<arith::AddFOp>(loc, remainder, rhs);
                    b.create<scf::YieldOp>(loc, ValueRange(fixedRemainder));
                  },
                  // else branch
                  [&](OpBuilder &b, Location loc) {
                    b.create<scf::YieldOp>(loc, ValueRange(remainder));
                  })
                 .getResult(0);
  } else {
    assert(dtype.isInteger() &&
    ...
    result = b.create<scf::IfOp>(
                  ...
                  [&](OpBuilder &b, Location loc) {
                    Value fixedRemainder =
                        b.create<arith::AddIOp>(loc, remainder, rhs);
                    b.create<scf::YieldOp>(loc, ValueRange(fixedRemainder));
                  },
                  // else branch
                  [&](OpBuilder &b, Location loc) {
