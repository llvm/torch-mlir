...
public:
  ...
  matchAndRewrite(AtenUnfoldOp op, OpAdaptor adaptor,
    ...
    // Calculate number of blocks from unfold:
    ...
    for (int64_t i = 0; i < inputRank; i++) {
      outputShape.push_back(rewriter.create<tensor::DimOp>(loc, input, i));
    }
    ...
    for (int64_t i = 0; i < numBlocks; i++) {
      ...
      offsets[dimension] = rewriter.getIndexAttr(offset);
      sizes[dimension] = rewriter.getIndexAttr(size);
      strides[dimension] = rewriter.getIndexAttr(step);

      // Extract slice from input
      Value slice = rewriter.create<tensor::ExtractSliceOp>(loc, input, offsets,
                                                            sizes, strides);

      // Insert slice into output
      SmallVector<OpFoldResult> insertOffsets(outputShape.size(),
                                              rewriter.getIndexAttr(0));
      ;
      insertOffsets[dimension] = rewriter.getIndexAttr(i * step);
      rewriter.create<tensor::InsertSliceOp>(loc, slice, outputTensor,
                                             insertOffsets, sizes, strides);
    }
