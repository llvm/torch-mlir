...
public:
  ...
  matchAndRewrite(OperatorOp op, OpAdaptor adaptor,
    ...
    if (failed(getConstantIntegerFromDefiningOp(unpacked_type_width,
                                                unpackedBitWidth))) {
      return failure();
    }
    ...
    RankedTensorType lhs_expandedType =
        RankedTensorType::get(lhs_expandedShape, elementType);
    ...
    // expand rhs
    std::vector<int64_t> expandedShape = {rhsShape[0], rhs_reduct_dim_size / gs,
                                          gs};
    RankedTensorType expandedType =
        RankedTensorType::get(expandedShape, rhs_elementType);
    ...
    Value dq_empty =
        rewriter.create<tensor::EmptyOp>(loc, expandedShape, elementType);
    ...
    Value empty = rewriter.create<tensor::EmptyOp>(loc, resultShape,
                                                   elementType, dynDims);
    Value output =
        rewriter.create<linalg::FillOp>(loc, cst_0, empty).getResult(0);
    ...
    SmallVector<utils::IteratorType> dq_iteratorTypes(
    ...
    Value dq_rhs =
        rewriter
            .create<linalg::GenericOp>(
                loc, dq_empty.getType(), ValueRange{expanded_rhs, scales, zps},
                dq_empty,
    ...
    Value quantMat =
        rewriter
            .create<linalg::GenericOp>(
                loc, output.getType(), ValueRange{expanded_lhs, dq_rhs}, output,
