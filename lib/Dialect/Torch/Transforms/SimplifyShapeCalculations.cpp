...
// TODO: This should use op traits which should be mostly autogenerated from
// the registry.
static bool isListPotentiallyMutated(Value list) {
  assert(list.getType().isa<Torch::ListType>());
  return !llvm::all_of(list.getUsers(), [](Operation *user) {
    // A simple allowlist of ops that we know don't mutate the list.
    return isa<AtenEqIntListOp, Aten__Getitem__TOp,
               ShapeCalculateYieldShapesOp>(user);
  });
}
