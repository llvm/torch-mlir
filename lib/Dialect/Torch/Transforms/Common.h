...
#define llvm_assert_ret(exp, ret, ...) if(exp){ printf(__VA_ARGS__); return ret; }
#define input_assert_ret(exp, ret, ...) llvm_assert_ret(exp, ret, "input error, require: " __VA_ARGS__)
//debug macro
...
public:
  RewriteOp(MLIRContext *context, AtenConvolutionOp &convOp) :
    context(context), rewriter(context), convOp(convOp), loc(convOp.getLoc()) {
  ...
  ValueTensorType getValueTensorType(vector<int64_t> shape) {
    return ValueTensorType::get(context, llvm::ArrayRef(shape), rewriter.getF32Type());
  }
  ...
  DenseElementsAttr getTensorDense(vector<int64_t> shape, vector<float> tensor) {
    return DenseElementsAttr::get(RankedTensorType::get(llvm::ArrayRef(shape), rewriter.getF32Type()), llvm::ArrayRef(tensor));
  }
  ...
  Value createIntOp(int64_t value) {
    return rewriter.create<ConstantIntOp>(loc, rewriter.getI64IntegerAttr(value));
  }
  Value createFloatOp(double value) {
    return rewriter.create<ConstantFloatOp>(loc, rewriter.getF64FloatAttr(value));;
  }
  Value createConvOp(Type result, Value inputOp, Value weightOp, Value biasOp, Value groupOp) {
    return rewriter.create<AtenConvolutionOp>(
        loc, result, inputOp, weightOp, biasOp, convOp.getOperand(3), 
        convOp.getOperand(4), convOp.getOperand(5), 
        convOp.getOperand(6), convOp.getOperand(7), groupOp);
  }
  Value createConvOp(Type result, Value inputOp, Value weightOp, Value biasOp) {
    return createConvOp(result, inputOp, weightOp, biasOp, convOp.getOperand(8));
  }
  Value createConvOp(Value inputOp, Value weightOp, Value biasOp, Value groupOp) {
  ...
  void replaceConvOp(Value newInputOp) {
    Value newConv = createConvOp(convOp.getType(), newInputOp, oldKernelOp, oldBiasOp);
  ...
  Value createAddTensorOp(Type result, Value tensor1, Value tensor2, Value alpha) {
    return rewriter.create<AtenAddTensorOp>(loc, result, tensor1, tensor2, alpha);
  }
  Value createSliceTensorOp(vector<int64_t> branchShape, Value input, Value dim, Value start, Value end) {
    ...
    return rewriter.create<AtenSliceTensorOp>(loc, branchTensorType, input, dim, start, end, step);
  }
  Value creatTensorListOp(ValueTensorType tensorType, vector<Value> tensorVec) {
    return rewriter.create<PrimListConstructOp>(loc, ListType::get(tensorType), ValueRange(tensorVec));
  }
  Value creatCatTensorOp(vector<int64_t> resultShape, Value dim, vector<Value> tensorVec) {
...
#define handle_param2(micro, type1, param1, type2, param2) micro(type1, param1), micro(type2, param2)
...
namespace {                                             \
  ...
  public:                                               \
    ...
    void runOnOperation() override {                    \
      ...
      auto f = getOperation();                          \
      name(context, f,                                  \
        handle_param(n, this_param, __VA_ARGS__));      \
    }                                                   \
  };                                                    \
}                                                       \
