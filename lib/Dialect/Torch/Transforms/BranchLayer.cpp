...
static void BranchLayer(MLIRContext *context, Operation *f, int layer, int branch) {
  ...
  for (int i = 0; i < branch; i++) {
    ...
    branchTensorOp[i] = rewrite.createSliceTensorOp(branchShape[i], oldInputOp, dimOp, startOp, endOp);
  }
  ...
  for (int i = 0; i < branch; i++) {
    ...
    // insert new conv
    branchTensorOp[i] = rewrite.createConvOp(branchTensorOp[i], oneKernel, zeroBias);
  }
