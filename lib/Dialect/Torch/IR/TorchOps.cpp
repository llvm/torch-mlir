...
OpFoldResult AtenItemOp::fold(FoldAdaptor adaptor) {
  ...
  // see if we have a tensor generated by Ones, Zeros, or Full
  {
    // get the appropriate scalar return type
    auto tensorType = getOperand().getType().dyn_cast<BaseTensorType>();
    if (!tensorType)
      return nullptr;
    if(!tensorType.hasDtype()){
      return nullptr;
    ...
    // if input is generated by Ones, Zeros, or Full, return the constant value
    if (auto onesOp = getSelf().getDefiningOp<AtenOnesOp>()) {
      // return one
      if (scalarType.isa<mlir::IntegerType>()) {
        return getI64IntegerAttr(getContext(), 1);
      }
      if (scalarType.isa<mlir::FloatType>()) {
        return getF64FloatAttr(getContext(), 1.0);
      }
    }
    if (auto zerosOp = getSelf().getDefiningOp<AtenZerosOp>()) {
      // return zero
      if (scalarType.isa<mlir::IntegerType>()) {
        return getI64IntegerAttr(getContext(), 0);
      }
    ...
    if (auto fullOp = getSelf().getDefiningOp<AtenFullOp>()) {
      // return the fill value
      if (scalarType.isa<mlir::IntegerType>()) {
        int64_t fillValue;
        if (matchPattern(fullOp.getFillValue(), m_TorchConstantInt(&fillValue))) {
          return getI64IntegerAttr(getContext(), fillValue);
        }
      }
      if (scalarType.isa<mlir::FloatType>()) {
        double fillValue;
        if (matchPattern(fullOp.getFillValue(), m_TorchConstantFloat(&fillValue))) {
          return getF64FloatAttr(getContext(), fillValue);
        }
      }
    }
  }
