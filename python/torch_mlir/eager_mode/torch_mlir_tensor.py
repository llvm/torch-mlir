        ...
        try:
            with no_dispatch():
                ...
                if "detach" in op_name:
                    # We don't handle detach as it only pertains to autograd graph construction,
                    # which is handled by pytorch.
                    raise UnsupportedByTorchMlirEagerMode("detaching")

                if not hasattr(func, "_schema"):
                    raise RuntimeError(f"op {func} has no schema.")

                normalized_kwargs = normalize_args_kwargs(
                    func.overloadpacket, args, kwargs
                )
            ...
            out = op_mlir_backend_callable(*tensor_args)

            maybe_aliased_arg_name = check_get_aliased_arg(func)
            if maybe_aliased_arg_name is not None:
                backend.copy_into(normalized_kwargs[maybe_aliased_arg_name], out)
