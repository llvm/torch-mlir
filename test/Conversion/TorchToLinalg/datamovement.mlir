...
// CHECK-LABEL: func.func @torch.aten.col2im_f32(
...
// CHECK: %[[VAL_1:.*]] = linalg.fill ins(%[[VAL_CST:.*]] : f32) outs(%[[VAL_0]] : tensor<1x3x16x32xf32>) -> tensor<1x3x16x32xf32>
// CHECK: %[[VAL_2:.*]] = torch_c.to_builtin_tensor %[[VAL_ARG0]] : !torch.vtensor<[1,12,128],f32> -> tensor<1x12x128xf32>
// CHECK: %[[VAL_3:.*]] = tensor.empty() : tensor<2x2xf32>
// CHECK: %[[VAL_4:.*]] = tensor.empty() : tensor<8x16xf32>
// CHECK: %[[VAL_5:.*]] = linalg.generic {indexing_maps = [#[[MAP]], #[[MAP1]], #[[MAP2]], #[[MAP3]]], iterator_types = ["parallel", "parallel", "reduction", "reduction", "reduction", "reduction"]} ins(%[[VAL_2]], %[[VAL_3]], %[[VAL_4]] : tensor<1x12x128xf32>, tensor<2x2xf32>, tensor<8x16xf32>) outs(%[[VAL_1]] : tensor<1x3x16x32xf32>) {
...
func.func @torch.aten.col2im_f32(%arg0: !torch.vtensor<[1,12,128],f32>) -> !torch.vtensor<[1,3,14,30],f32> {
...
// CHECK: #[[MAP2:.*]] = affine_map<(d0, d1, d2, d3, d4, d5) -> (d2, d3)>
// CHECK: #[[MAP3:.*]] = affine_map<(d0, d1, d2, d3, d4, d5) -> (d0, d1, d2 * 2 + d4, d3 * 2 + d5)>
// CHECK-LABEL: func.func @torch.aten.col2im_si32(
// CHECK-SAME:                                %[[VAL_ARG0:.*]]: !torch.vtensor<[1,12,128],si32>) -> !torch.vtensor<[1,3,14,30],si32> {
// CHECK: %[[VAL_CST:.*]] = arith.constant 0 : i32
// CHECK: %[[VAL_0:.*]] = tensor.empty() : tensor<1x3x16x32xi32>
// CHECK: %[[VAL_1:.*]] = linalg.fill ins(%[[VAL_CST:.*]] : i32) outs(%[[VAL_0]] : tensor<1x3x16x32xi32>) -> tensor<1x3x16x32xi32>
// CHECK: %[[VAL_2:.*]] = torch_c.to_builtin_tensor %[[VAL_ARG0]] : !torch.vtensor<[1,12,128],si32> -> tensor<1x12x128xi32>
// CHECK: %[[VAL_3:.*]] = tensor.empty() : tensor<2x2xi32>
// CHECK: %[[VAL_4:.*]] = tensor.empty() : tensor<8x16xi32>
// CHECK: %[[VAL_5:.*]] = linalg.generic {indexing_maps = [#[[MAP]], #[[MAP1]], #[[MAP2]], #[[MAP3]]], iterator_types = ["parallel", "parallel", "reduction", "reduction", "reduction", "reduction"]} ins(%[[VAL_2]], %[[VAL_3]], %[[VAL_4]] : tensor<1x12x128xi32>, tensor<2x2xi32>, tensor<8x16xi32>) outs(%[[VAL_1]] : tensor<1x3x16x32xi32>) {
// CHECK: ^bb0(%[[VAL_IN0:.*]]: i32, %[[VAL_IN1:.*]]: i32, %[[VAL_IN2:.*]]: i32, %[[VAL_OUT:.*]]: i32):
// CHECK:   %[[VAL_7:.*]] = arith.addi %[[VAL_IN0]], %[[VAL_OUT]] : i32
// CHECK:   linalg.yield %[[VAL_7]] : i32
// CHECK: } -> tensor<1x3x16x32xi32>
// CHECK: %[[VAL_SLICE:.*]] = tensor.extract_slice %[[VAL_5]][0, 0, 1, 1] [1, 3, 14, 30] [1, 1, 1, 1] : tensor<1x3x16x32xi32> to tensor<1x3x14x30xi32>
// CHECK: %[[VAL_6:.*]] = torch_c.from_builtin_tensor %[[VAL_SLICE]] : tensor<1x3x14x30xi32> -> !torch.vtensor<[1,3,14,30],si32>
// CHECK: return %[[VAL_6]] : !torch.vtensor<[1,3,14,30],si32>
func.func @torch.aten.col2im_si32(%arg0: !torch.vtensor<[1,12,128],si32>) -> !torch.vtensor<[1,3,14,30],si32> {
  %int14 = torch.constant.int 14
  %int30 = torch.constant.int 30
  %0 = torch.prim.ListConstruct %int14, %int30 : (!torch.int, !torch.int) -> !torch.list<int>
  %int2 = torch.constant.int 2
  ...
  %int2_5 = torch.constant.int 2
  %4 = torch.prim.ListConstruct %int2_4, %int2_5 : (!torch.int, !torch.int) -> !torch.list<int>
  %5 = torch.aten.col2im %arg0, %0, %1, %2, %3, %4 : !torch.vtensor<[1,12,128],si32>, !torch.list<int>, !torch.list<int>, !torch.list<int>, !torch.list<int>, !torch.list<int> -> !torch.vtensor<[1,3,14,30],si32>
  return %5 : !torch.vtensor<[1,3,14,30],si32>
}
...
// CHECK: #[[MAP2:.*]] = affine_map<(d0, d1, d2, d3, d4, d5) -> (d2, d3)>
// CHECK: #[[MAP3:.*]] = affine_map<(d0, d1, d2, d3, d4, d5) -> (d0, d1, d2 * 2 + d4, d3 * 2 + d5)>
// CHECK-LABEL: func.func @torch.aten.col2im_complex(
// CHECK-SAME:                                %[[VAL_ARG0:.*]]: !torch.vtensor<[1,12,128],complex<f32>>) -> !torch.vtensor<[1,3,14,30],complex<f32>> {
// CHECK: %[[VAL_CST:.*]] = arith.constant 0.000000e+00 : f32
// CHECK: %[[VAL_0:.*]] = tensor.empty() : tensor<1x3x16x32xcomplex<f32>>
// CHECK: %[[VAL_CST_COMPLEX:.*]] = complex.create %[[VAL_CST]], %[[VAL_CST]] : complex<f32>
// CHECK: %[[VAL_1:.*]] = linalg.fill ins(%[[VAL_CST:.*]] : complex<f32>) outs(%[[VAL_0]] : tensor<1x3x16x32xcomplex<f32>>) -> tensor<1x3x16x32xcomplex<f32>>
// CHECK: %[[VAL_2:.*]] = torch_c.to_builtin_tensor %[[VAL_ARG0]] : !torch.vtensor<[1,12,128],complex<f32>> -> tensor<1x12x128xcomplex<f32>>
// CHECK: %[[VAL_3:.*]] = tensor.empty() : tensor<2x2xcomplex<f32>>
// CHECK: %[[VAL_4:.*]] = tensor.empty() : tensor<8x16xcomplex<f32>>
// CHECK: %[[VAL_5:.*]] = linalg.generic {indexing_maps = [#[[MAP]], #[[MAP1]], #[[MAP2]], #[[MAP3]]], iterator_types = ["parallel", "parallel", "reduction", "reduction", "reduction", "reduction"]} ins(%[[VAL_2]], %[[VAL_3]], %[[VAL_4]] : tensor<1x12x128xcomplex<f32>>, tensor<2x2xcomplex<f32>>, tensor<8x16xcomplex<f32>>) outs(%[[VAL_1]] : tensor<1x3x16x32xcomplex<f32>>) {
// CHECK: ^bb0(%[[VAL_IN0:.*]]: complex<f32>, %[[VAL_IN1:.*]]: complex<f32>, %[[VAL_IN2:.*]]: complex<f32>, %[[VAL_OUT:.*]]: complex<f32>):
// CHECK:   %[[VAL_7:.*]] = complex.add %[[VAL_IN0]], %[[VAL_OUT]] : complex<f32>
// CHECK:   linalg.yield %[[VAL_7]] : complex<f32>
// CHECK: } -> tensor<1x3x16x32xcomplex<f32>>
// CHECK: %[[VAL_SLICE:.*]] = tensor.extract_slice %[[VAL_5]][0, 0, 1, 1] [1, 3, 14, 30] [1, 1, 1, 1] : tensor<1x3x16x32xcomplex<f32>> to tensor<1x3x14x30xcomplex<f32>>
// CHECK: %[[VAL_6:.*]] = torch_c.from_builtin_tensor %[[VAL_SLICE]] : tensor<1x3x14x30xcomplex<f32>> -> !torch.vtensor<[1,3,14,30],complex<f32>>
// CHECK: return %[[VAL_6]] : !torch.vtensor<[1,3,14,30],complex<f32>>
func.func @torch.aten.col2im_complex(%arg0: !torch.vtensor<[1,12,128],complex<f32>>) -> !torch.vtensor<[1,3,14,30],complex<f32>> {
  %int14 = torch.constant.int 14
  %int30 = torch.constant.int 30
  %0 = torch.prim.ListConstruct %int14, %int30 : (!torch.int, !torch.int) -> !torch.list<int>
  %int2 = torch.constant.int 2
  ...
  %int2_5 = torch.constant.int 2
  %4 = torch.prim.ListConstruct %int2_4, %int2_5 : (!torch.int, !torch.int) -> !torch.list<int>
  %5 = torch.aten.col2im %arg0, %0, %1, %2, %3, %4 : !torch.vtensor<[1,12,128],complex<f32>>, !torch.list<int>, !torch.list<int>, !torch.list<int>, !torch.list<int>, !torch.list<int> -> !torch.vtensor<[1,3,14,30],complex<f32>>
  return %5 : !torch.vtensor<[1,3,14,30],complex<f32>>
}
