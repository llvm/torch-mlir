...
def path_based_shape_inference_model() -> onnx.ModelProto:
    ...
    large_tensor = numpy.random.rand(onnx.checker.MAXIMUM_PROTOBUF).astype(numpy.float32)
    ...
    X = make_tensor_value_info("large_const", TensorProto.FLOAT, [onnx.checker.MAXIMUM_PROTOBUF])
...
class CommandLineTest(unittest.TestCase):
    ...
    def run_model_explicit_temp_implicit_data(self, onnx_model: onnx.ModelProto, model_name: str):
        run_path = self.get_run_path(model_name)
        ...
        temp_dir = run_path / "temp"
        temp_dir.mkdir(exist_ok=True)
        args = __main__.parse_arguments([str(model_file), "-o", str(mlir_file), "--temp-dir", str(temp_dir)])
        __main__.main(args)
